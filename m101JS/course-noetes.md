...
## Resources
- BSON: bsonspec.org/spec.html


# Week 2: CRUD
## id Field

![Parts of ObjectId](object_id-parts.png)

## Matching Arrays
match the entire array
- arrayFieldName: ["item", "item", ...]
match if the item is in the array
- arrayFieldName: "item"
item as first item in array
- arrayFieldName.0: "item"
...
```
how many objects in batch
- may need to do before looping through
```js
c.objsLeftInBatch()
```
...
- id needs to be explicitly excluded
```js
...
### ex all movies with runtime > 90 min
```js
db.movieDetails.find({ runtime: { $gt: 90 } })
```
### $and
- $and is implicit. You only need to use it when you are anding the same field > 1 time (because keys in a JSON document must be unique)
